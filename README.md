...
**GlowBridge** is a lightweight, low-latency Python ambilight-style system that captures video from a V4L2 device (e.g., HDMI-to-USB), samples edge colors, and streams them to a **WLED** controller via UDP Realtime (DRGB).

It’s designed to be simple, hackable, low-CPU (Raspberry Pi friendly), and free of proprietary software or cloud dependencies.
...
| ---------------------- | --------------------------- | ------------------- | ----------------------------- |
| Raspberry Pi           | Pi 3B+ / Pi 4 / Pi Zero 2 W | $15–$45             | Pi 3+ recommended for Ethernet |
| HDMI → USB Capture     | Generic UVC dongle          | $8–$15              | Must support MJPEG            |
| LED Strip              | WS2812B / SK6812            | $5–$10              | ~150–300 LEDs typical         |
| WLED Controller        | ESP32-WROOM                 | $3–$6               | ESP8266 also works            |
| Power Supply (LEDs)    | 12V 2A+                     | $10–$20             | Depends on LED count          |
...
- WLED: `wled.ip`, `wled.port`, `wled.timeout_seconds`
...
- No Realtime in WLED: Ensure UDP Notifier is enabled and not blocked by firewall; confirm IP/port.
